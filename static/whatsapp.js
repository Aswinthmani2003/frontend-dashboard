...
function getAvatarColor(name) {
    if (!name) return 2;
    return Math.abs(name.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0)) % 8;
}
...
function isSameDate(timestamp, filterDate) {
    ...
    return messageDate.getFullYear() === filter.getFullYear() &&
...
async function loadAlertsFromBackend() {
    try {
        ...
        if (data.alerts && Array.isArray(data.alerts)) {
            ...
            console.log(`📥 Loaded ${data.alerts.length} persisted alerts from backend`);
        }
    } catch (error) {
        ...
        newContacts.forEach(contact => {
            ...
            if (oldContact) {
                if (contact.last_time !== oldContact.last_time) {
                    console.log(`🔔 New activity for ${contact.client_name || contact.phone}`);

                    if (contact.phone !== state.selectedPhone &&
                        (contact.last_direction === 'user' || contact.last_direction === 'incoming')) {
...
async function checkAndSetNewMessageAlert(phone) {
    try {
        ...
        if (!data.automation_enabled) {
            ...
            console.log(`⚪ Message received for ${phone} but Chatbot is ON - no alert`);
        }
    } catch (error) {
...
function updatePaginationInfo() {
    ...
    document.getElementById('paginationInfo').textContent = `Showing ${start}–${end}`;
    ...
    console.log(`📄 Pagination: Showing ${start}-${end}, Offset: ${state.offset}, Limit: ${state.limit}`);
}
...
async function loadAutomationStatus(phone) {
    try {
        const response = await fetch(`/api/automation/${phone}`);
        const data = await response.json();
        document.getElementById('automationToggle').checked = data.automation_enabled;
...
function renderContacts() {
    ...
    if (filteredContacts.length === 0) {
        container.innerHTML = '<div style="padding: 20px; text-align: center; color: #667781;">No contacts found</div>';
        return;
    }
    ...
    filteredContacts.forEach(contact => {
        ...
        const preview = contact.last_message ? 
            escapeHtml(contact.last_message.substring(0, 40)) + (contact.last_message.length > 40 ? '...' : '') :
            'No messages';
        ...
        div.innerHTML = `
            ...
            <div class="chat-info">
                ...
                <div class="chat-preview">
                    ${preview}
                    ${showNewMessageBadge ? '<span class="unread-badge">1</span>' : ''}
                </div>
            </div>
        `;
...
function renderMessages() {
    ...
    state.messages.forEach(msg => {
        ...
        if (msg.handled_by) {
            metaHtml += `<div class="message-meta">👤 ${escapeHtml(msg.handled_by)}</div>`;
        }
...
`;
    ...
    if (state.messages.length > 0) {
        ...
        document.getElementById('followUpToggle').checked = latest.follow_up_needed || false;
    }
...
document.getElementById('followUpToggle').addEventListener('change', async (e) => {
    if (!state.selectedPhone || !state.messages || state.messages.length === 0) {
...
document.getElementById('emojiPicker').addEventListener('click', (e) => {
    if (e.target.tagName === 'SPAN' || (e.target.textContent && e.target.textContent.match(/[\u{1F600}-\u{1F64F}]/u))) {
